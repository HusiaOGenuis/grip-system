...
def load_all_configuration() -> dict:
    return {
        ...
        "override_policy": load_override_policy(),
    }

def load_all_configuration() -> dict:
    """
    Single authoritative entry point for configuration activation.
    """
    return {
        "verdict_taxonomy": load_verdict_taxonomy(),
        "pause_criteria": load_pause_criteria(),
    }
def load_override_policy() -> dict:
    data = _load_yaml(CONFIG_DIR / "override_policy.yaml")

    if "roles" not in data:
        raise ConfigurationError("Override policy must define roles")

    return data
def load_confidence_override_policy() -> dict:
    data = _load_yaml(CONFIG_DIR / "confidence_override_policy.yaml")

    if "bands" not in data:
        raise ConfigurationError("Confidence override policy must define bands")
...
def load_all_configuration() -> dict:
    return {
        ...
        "pause_criteria": load_pause_criteria(),
        "override_policy": load_override_policy(),
        "confidence_override_policy": load_confidence_override_policy(),
    }
